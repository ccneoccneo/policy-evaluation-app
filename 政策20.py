...
class PolicyEvaluationPipeline:
    ...
    def run_naive_did(self, policy_name: str, policy_year: int,
        ...
        # 生成 LLM 解读（使用与命令行版本完全一致的 prompt）
        if self.llm:
            prompt = f"""
你是专业的政策评估研究员。请基于普通DID方法的DID分析结果，撰写政府研究报告风格的政策解读，分三段：
1. 政策效果总结（方向、大小、显著性）
2. 可能的影响机制或解释
3. 三条具体政策建议
...
P值：{did_res['p_value']}（{'显著' if did_res['significant'] else '不显著'}）
样本量：{did_res['n_obs']}
R²：{did_res['r_squared']}
...
"""
    ...
    def run_psm_did(self, policy_name: str, policy_year: int,
        ...
        if self.llm:
            prompt = f"""
你是专业的政策评估研究员。请基于PSM-DID方法的DID分析结果，撰写政府研究报告风格的政策解读，分三段：
1. 政策效果总结（方向、大小、显著性）
2. 可能的影响机制或解释
3. 三条具体政策建议
