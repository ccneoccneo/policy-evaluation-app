...
class CausalInferenceAnalyzer:
    ...
    def event_study(df: pd.DataFrame, outcome: str, treat_col: str,
        ...
        # 构建回归公式
        dummy_terms = " + ".join(time_dummies.values())
        formula = f"{outcome} ~ {dummy_terms}"
        # 加入控制变量
        if controls:
            formula += " + " + " + ".join(controls)
        # 加入个体固定效应（虚拟变量）
        ...
        try:
            ...
            for t, dummy in time_dummies.items():
                coefs[t] = res.params[dummy]
                ci = res.conf_int().loc[dummy]
                ci_low[t] = ci[0]
                ci_high[t] = ci[1]
        except Exception as e:
            logger.error(f"事件研究回归失败: {e}")
            return {}
        return {
            "coefficients": coefs,
            "ci_low": ci_low,
...
class PolicyEvalInterpreter:
    ...
    def interpret_did(self, method_name: str, did_result: Dict, policy_name: str, outcome_name: str,
        ...
        if parallel_trend_result and "coefficients" in parallel_trend_result:
            ...
            any_sig = any(ci_low[t] > 0 or ci_high[t] < 0 for t in pre_times)
...
if __name__ == "__main__":
    ...
    # ========== 参数设置（根据 Stata 代码中的变量名） ==========
    POLICY_NAME = "智能制造政策"
    POLICY_YEAR = 2015          # 政策实施年份（根据实际调整，示例中安慰剂检验用了2015-2018）
    TREAT_COL = "DID"           # 处理变量（注意大写）
    OUTCOME_COL = "RES"         # 结果变量：供应链韧性
    ...
    # 1. 普通DID多规格回归（逐步加入固定效应）
    print("\n=== 普通DID多规格回归 ===")
    fe_specs = [
        [],                     # 无固定效应
        ['stkcd'],              # 企业固定效应
        ['stkcd', 'year'],      # 企业+年份
        ['stkcd', 'year', 'city']  # 企业+年份+城市（如果数据有 city 列）
    ]
    ...
    if naive_event['llm_interpretation']:
        print(f"LLM解读: {naive_event['llm_interpretation']}")
    ...
    if psm_event['llm_interpretation']:
        print(f"LLM解读: {psm_event['llm_interpretation']}")
    # 3. PSM-DID + 事件研究图
    print("\n=== DML ===")
    dml_event = pipeline.run_dml(
        POLICY_NAME, POLICY_YEAR, TREAT_COL, OUTCOME_COL, CONTROL_COLS, MATCH_VARS
    )
    print(f"DML系数: {psm_event['did_results']['did_coefficient']}, p={psm_event['did_results']['p_value']}")
    if dml_event['llm_interpretation']:
        print(f"LLM解读: {psm_event['llm_interpretation']}")

    # 4. 中介效应检验（例如知识重构深度 DEPTH）
    # 注意：需要数据中有 DEPTH 和 EXTEND 列
    # if "DEPTH" in pipeline.preparer.df.columns:
    #     print("\n=== 中介效应检验（DEPTH） ===")
    #     mediation_res = pipeline.run_mediation(
    #         treatment="did_interaction", mediator="DEPTH", outcome=OUTCOME_COL,
    #         controls=CONTROL_COLS, fixed_effects=['stkcd', 'year']
    #     )
    #     print(f"间接效应: {mediation_res['indirect_effect']:.4f}, Sobel p={mediation_res['sobel_p']:.4f}")

    # 5. 调节效应检验（例如 DID × DIG）
    # if "DIG" in pipeline.preparer.df.columns:
    #     print("\n=== 调节效应检验（数字化转型 DIG） ===")
    #     interaction_res = pipeline.run_interaction(
    #         treatment="did_interaction", moderator="DIG", outcome=OUTCOME_COL,
    #         controls=CONTROL_COLS, fixed_effects=['stkcd', 'year']
    #     )
    #     print(f"交互项系数: {interaction_res['coefficient']:.4f}, p={interaction_res['p_value']:.4f}")
