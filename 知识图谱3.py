...
# ==================== 7. 美化可视化 ====================
fig, ax = plt.subplots(figsize=(26, 20))  # 增大画布尺寸
...
node_sizes = [pagerank[n]*3500 + 600 for n in G.nodes()]  # 加基础尺寸，确保小节点也可见
...
# 标签：调整字体大小，对于关键节点可以加大字号
labels = {}
label_font_sizes = {}
for n in G.nodes():
    labels[n] = G.nodes[n]['name']
    # 根据PageRank动态调整字号：重要性高的节点字体稍大
    if pagerank[n] > 0.08:
...
# 标题，使用 subplots_adjust 留出足够顶部空间
