...
安装依赖：
  pip install pandas numpy statsmodels linearmodels econml matplotlib seaborn scikit-learn
"""
...
class PolicyEvaluationPipeline:
    ...
    def run_did_evaluation(self, policy_name: str, policy_year: int,
        ...
        # Step 6: 保存结果（修复 JSON 序列化问题）
        def make_json_serializable(obj):
            """递归转换对象，使其可被 JSON 序列化"""
            ...
            elif hasattr(obj, 'item'):  # 处理标量 numpy 类型
                return obj.item()
            else:
                return obj

        # 排除 parallel_trend 是因为它包含大量列表数据，但也可以序列化；这里保留原逻辑排除
        result_to_save = {k: v for k, v in result.items() if k != "parallel_trend"}
        serializable_result = make_json_serializable(result_to_save)
