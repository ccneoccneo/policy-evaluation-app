"""
政策效果自动评估流水线（最终版：普通DID多规格、PSM-DID、DML）
适配 Stata / CSV 数据格式（stkcd, year, DID, RES, 控制变量等）
删除事件研究法，仅保留核心计量分析 + 综合LLM解读（含政府/企业建议）
"""
            ...
            messages = [
                {
                    "role": "system",
                    "content": (
                        "你是一个专业的政策评估研究员，擅长将计量经济学结果转化为政府研究报告风格的语言。"
                        "请用中文回答。"
                    ),
                },
...
class PolicyDataPreparer:
    def __init__(self, data_path: str):
        ...
        if "did" in self.df.columns:
            ...
            raise ValueError("缺少处理组列（did/treat/DID）")
        ...
        logger.info(
            "列名映射: time=%s, entity=%s, treat=%s, city=%s",
            self.col_map["time"],
            self.col_map["entity"],
            self.col_map["treat"],
            self.col_map["city"],
    ...
    def prepare_dml_data(
        ...
        for col in [outcome_col, treat_col] + feature_cols:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce")
                df[f"{col}_demeaned"] = df.groupby(actual_entity_col)[col].transform(
                    lambda x: x - x.mean()
                )
        ...
        if y_col not in df.columns:
            raise ValueError(f"DML 结果变量不存在：{y_col}")
        if t_col not in df.columns:
            raise ValueError(f"DML 处理变量不存在：{t_col}")
        if len(x_cols) == 0:
            raise ValueError("DML 特征变量为空，请检查 feature_cols 是否有效。")
...
class CausalInferenceAnalyzer:
    ...
    def psm_matching(
        ...
        matched_df = pd.concat([treat_df, matched_ctrl], axis=0)
    ...
    def did_regression(
        ...
        try:
            ...
            if city_col and city_col in working_df.columns:
                ...
                controls_local += list(city_dummies.columns)
            ...
            if entity_col and time_col:
                ...
                rhs_terms = [did_col]
                if controls_local:
                    rhs_terms.extend(controls_local)
                if fe_terms:
...
class DoubleMachineLearningAnalyzer:
    def __init__(self, model_y=None, model_t=None, n_folds: int = 5):
    ...
    def _get_default_models(self):
        try:
            ...
            return (
                RandomForestRegressor(n_estimators=100, min_samples_leaf=10, random_state=42),
    ...
    def fit(self, Y: np.ndarray, T: np.ndarray, X: np.ndarray):
        ...
        if denom < 1e-10:
            logger.warning("T残差平方和过小，添加微小噪声")
    ...
    def get_ate_results(self) -> Dict:
        ...
        return {
            "ate_coefficient": round(float(self._ate), 4) if pd.notna(self._ate) else np.nan,
            "std_error": round(float(self._ate_std), 4) if pd.notna(self._ate_std) else np.nan,
            ...
            "ci_lower": round(float(self._ate_ci[0]), 4) if pd.notna(self._ate_ci[0]) else np.nan,
            "ci_upper": round(float(self._ate_ci[1]), 4) if pd.notna(self._ate_ci[1]) else np.nan,
...
class PolicyEvaluationPipeline:
    def __init__(self, data_path: str, llm_engine: Optional[QwenEngine] = None):
    ...
    def run_naive_did(
        ...
        interpretation = self.interpreter.interpret_did("普通DID", did_res, policy_name, outcome_col) if self.interpreter else ""
    ...
    def run_psm_did(
        ...
        interpretation = self.interpreter.interpret_did("PSM-DID", did_res, policy_name, outcome_col) if self.interpreter else ""
        return {"did_results": did_res, "llm_interpretation": interpretation}
    ...
    def _generate_comparison_conclusion(self, naive: Dict, psm: Dict, dml: Dict) -> str:
        ...
        if dml_sig and (naive_sig or psm_sig):
            ...
            conclusion += " 传统DID显著但DML不显著，可能存在遗漏变量或非线性关系，建议谨慎解释并进一步检验。"
...
if __name__ == "__main__":
    ...
    DATA_PATH = "data2.csv"   # 请修改为您的数据文件路径

    api_key = os.getenv("DASHSCOPE_API_KEY")
    llm_engine = QwenEngine(api_key=api_key, model="qwen-turbo") if api_key else None
    ...
    for i, res in enumerate(multi_results):
        print(
            f"规格{i+1} {res['fixed_effects']}: "
            f"DID系数={res['did_coefficient']}, p={res['p_value']}, 显著={res['significant']}"
        )
    ...
    # 3. PSM-DID
    print("\n=== PSM-DID ===")
    psm_result = pipeline.run_psm_did(
        POLICY_NAME,
        POLICY_YEAR,
        TREAT_COL,
        OUTCOME_COL,
...
"""
        ...
        print("\n=== 未检测到 DASHSCOPE_API_KEY，已跳过 LLM 解读 ===")
